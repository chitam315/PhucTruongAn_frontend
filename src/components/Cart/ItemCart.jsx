...
export default function ItemCart({ item, idProduct,setUpdate }) {
  const { loadingDel, execute: deleteProductCart } = useAsync(() => {
  ...
  return (
    <div className="flex border-gray rounded-[10px] overflow-hidden p-[10px] mb-[15px] bg-[#fdfdfd]">
      ...
      <div className="w-100 flex flex-col justify-center ml-[15px] ">
        <Link to={`/detail/${item.product_id}`} className="flex-center flex-block mb-[15px]">
