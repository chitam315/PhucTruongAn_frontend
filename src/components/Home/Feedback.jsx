          ...
          {arr.map((item, key) => (
            <div className="card-shadow p-[20px] m-[5px] select-none">
              <div className="flex items-center">
                <img src={item.avatar} alt="" className="rounded-full w-[80px] h-[80px] border-2 border-solid border-[var(--mainColor)]" />
                <div className="ml-[15px] text-[1.2em]">
                  <p className="text-[var(--mainColor)] font-bold ">{item.name}</p>
                  <p className="text-[#535353] ">{item.career}</p>
                </div>
              </div>
              <p className="mt-[20px] text-[1em] text-justify text-[#545454]">{item.content}</p>
